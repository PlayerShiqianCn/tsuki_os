...
#define WINDOW_BUFFER_BASE  0x400000u
#define WINDOW_BUFFER_SLOTS 8
...
static unsigned char window_buffer_used[WINDOW_BUFFER_SLOTS];
...
static unsigned int* window_buffer_ptr(int slot) {
    return ((unsigned int*)WINDOW_BUFFER_BASE) + slot * WINDOW_BUFFER_PIXELS;
}

static int alloc_window_buffer_slot(void) {
    for (int i = 0; i < WINDOW_BUFFER_SLOTS; i++) {
...
static void release_window_buffer_slot(int slot) {
    if (slot < 0 || slot >= WINDOW_BUFFER_SLOTS) return;

...
typedef struct {
    ...
    unsigned int load_addr;     // 建议加载地址
    unsigned int entry_offset;  // 入口点相对于 load_addr 的偏移
    unsigned int image_size;    // 需要拷贝到内存的完整镜像大小
    unsigned int image_checksum;// FNV-1a 校验
    unsigned int flags;
